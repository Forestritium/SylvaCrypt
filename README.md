...
| **Key exchange** | Full X3DH (Double Ratchet Protocol): IK + SPK + OPK prekeys published to server; sender runs X3DH setup before first message to establish shared secret |
...
**Honest limitations:** no web app can block OS-level screen capture; capture deterrence raises friction but is not a security guarantee. The header encryption relies on Next Header Key (NHK) rotation on every DH ratchet step, providing full forward secrecy for metadata in the Double Ratchet. Sender identity is hidden via a sealed-sender box (ECDH ephemeral + AES-GCM) on every message, so the relay cannot link a delivery to a specific sender. See [SECURITY.md](SECURITY.md) for the full threat model including what SylvaCrypt does *not* protect against.
...
- **End-to-end encrypted** messages using the Double Ratchet (X25519 + HKDF-SHA256 + AES-256-GCM)
...
| Feature | Details |
|---|---|
| **End-to-End Encryption** | Double Ratchet Protocol — per-message unique keys, full forward secrecy |
| **X3DH Key Exchange** | Full Standard X3DH: IK + SPK + OPK prekeys; fresh shared secret per conversation |
...
| **Key Exchange** | Full X3DH (Double Ratchet Protocol) + ML-KEM-768 post-quantum hybrid |
| **Message Encryption** | Double Ratchet (AES-256-GCM + HKDF-SHA256) |
...
│   │   ├── doubleRatchet.ts  # Double Ratchet + NHK header key rotation
│   │   ├── relay.ts          # Supabase messaging relay & server sync
│   │   ├── localStore.ts     # Encrypted IndexedDB vault
│   │   ├── deviceStore.ts    # Multi-device session manager
