...
Welcome to the SylvaCrypt FAQ! Below are answers to the most common questions about our Zero-Knowledge Messaging Platform.
...
SylvaCrypt uses the Double Ratchet algorithm combined with Post-Quantum (ML-KEM-768) encapsulation to ensure forward secrecy and post-quantum security. We also use Ed25519 for identity verification and AES-256-GCM for symmetric message encryption.
...
If you lose your device, you can log in from a new device using your credentials and your recovery phrase. However, because we do not store your message history in plain text, you will only be able to see new messages unless you restore a backup from a secure local vault.

## Troubleshooting

**Why did my secure session reset?**
Sessions can occasionally get out of sync if there is an unrecoverable mismatch in the Double Ratchet state (for example, if a device goes offline for a very long time and misses too many messages). The app will actively error out if a downgrade attack is detected. If an unrecoverable state error occurs, users can re-establish the connection by initiating a new chat.
