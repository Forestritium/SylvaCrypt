...
SylvaCrypt is built around a zero-knowledge security model. This document describes the threat model, cryptographic design, known limitations, and the responsible disclosure process.
...
### What SylvaCrypt protects against
...
### What SylvaCrypt does NOT protect against

| Threat | Explanation |
|---|---|
| **Compromised device / malware** | If the device running SylvaCrypt is compromised, an attacker can read decrypted messages from memory. |
...
| **Metadata analysis** | SylvaCrypt hides message content but not the fact that two users are communicating or the frequency of communication. |
...
SylvaCrypt implements a full Double Ratchet Algorithm with post-quantum layer:
...
- **Initialisation**: X3DH (IK+SPK+OPK) shared secret combined with ML-KEM-768 shared secret via HKDF-SHA256 → initial root key.
- **Header encryption**: Envelope headers encrypted with a per-epoch Next Header Key (NHK) derived via HKDF("SylvaCrypt-InitV2") from the initial shared secret, rotating with each DH ratchet step. Hides ratchet metadata from the relay and provides full forward secrecy for metadata.
- **Sealed Sender**: Every message envelope is packed into a sealed-sender box (ECDH ephemeral + AES-GCM) so the relay cannot link a delivery to a specific sender.
...
- `Argon2id(normalised_mnemonic)` stored in `profiles.mnemonic_hash` (new v2 accounts). Legacy v0 accounts use `SHA-256(normalised_mnemonic)`.
...
3. **Static header key migration** — Historically, the header encryption key (HK) was derived once. SylvaCrypt v2.5.0+ fully rotates the Next Header Key (NHK) on every DH ratchet step, providing full forward secrecy for envelope metadata. Sender identity is separately protected by the sealed-sender box mechanism (see below), which prevents the relay from linking a delivered envelope to its sender.
...
6. **Self-hosted deployments** — If you self-host SylvaCrypt, you are responsible for securing your Supabase project, applying migrations, and keeping dependencies up to date.
