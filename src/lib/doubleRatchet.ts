...
/** Header Key ratchet step: HK_i -> HK_{i+1} */
async function stepHK(hkBase64: string): Promise<string> {
  const hk = fromBase64(hkBase64);
  const nextHk = await hmacSha256(hk, new Uint8Array([5]));
  return toBase64(nextHk);
}
...
export async function initSessionSender(
  ...
  return {
    conversationId,
    sharedSecret: toBase64(shared),
    DHs_priv: eph.privateKeyBase64,
    DHs_pub: eph.publicKeyBase64,
    DHr: theirPubB64,
    KEMs_priv: toBase64(kemKP.secretKey),
    KEMs_pub: toBase64(kemKP.publicKey),
    RK: toBase64(newRK),
    CKs: toBase64(cks),
    CKr: null,
    Ns: 0, Nr: 0, PN: 0,
...
async function tryDecryptHeader(hkB64: string, blob: string): Promise<PlaintextHeader | null> {
  try {
    ...
    return JSON.parse(decoded) as PlaintextHeader;
  } catch (e) {
    return null;
  }
}
...
function dhsPriv(s: RatchetSession): string {
  if ('DHs_priv' in s) return s.DHs_priv as string;
  // Fallback for migrated/legacy sessions (if any leak into runtime):
  return ((s as any).DHs?.split('|')[0] as string) || '';
}
function dhsPub(s: RatchetSession): string {
  if ('DHs_pub' in s) return s.DHs_pub as string;
  return ((s as any).DHs?.split('|')[1] as string) || '';
}

function kemsPriv(s: RatchetSession): string {
  if ('KEMs_priv' in s) return s.KEMs_priv as string;
  return ((s as any).KEMs?.split('|')[0] as string) || '';
}
function kemsPub(s: RatchetSession): string {
  if ('KEMs_pub' in s) return s.KEMs_pub as string;
  return ((s as any).KEMs?.split('|')[1] as string) || '';
}

function prevKemsPriv(s: RatchetSession): string | undefined {
  if ('prevKEMs_priv' in s) return s.prevKEMs_priv as string | undefined;
  if (!(s as any).prevKEMs) return undefined;
  return ((s as any).prevKEMs.split('|')[0] as string);
}
...
async function dhRatchetStep(
  ...
  if (kemCiphertextB64 && session.KEMs && session.KEMs.includes('|')) {
    ...
    try {
      ...
      const prevSecKeyBase64 = prevKemsPriv(session);
      if (prevSecKeyBase64) {
        try {
  ...
  let newKEMsPriv = kemsPriv(session);
  let newKEMsPub = kemsPub(session);

  const theirKEMPub = theirKEMPubB64 ?? session.KEMr;
  if (theirKEMPub) {
    try {
      const output = ml_kem768.encapsulate(fromBase64(theirKEMPub));
      // @noble/post-quantum returns { cipherText, sharedSecret }
      kemEncapsulated = output.sharedSecret || new Uint8Array();
      newKEMct = toBase64((output as any).cipherText || (output as any).ciphertext || new Uint8Array());
    } catch (e) {
  ...
  try {
    const kemKP = ml_kem768.keygen();
    newKEMsPriv = toBase64(kemKP.secretKey);
    newKEMsPub = toBase64(kemKP.publicKey);
  } catch (e) {
  ...
  return {
    ...
    PN: session.Ns,
    Ns: 0, Nr: 0,
    DHs_priv: newKP.privateKeyBase64,
    DHs_pub: newKP.publicKeyBase64,
    DHr: theirNewPubB64,
    KEMr: theirKEMPub,
    KEMs_priv: newKEMsPriv,
    KEMs_pub: newKEMsPub,
    prevKEMs_priv: kemsPriv(session),
    prevKEMs_pub: kemsPub(session),
    KEM_ct: newKEMct,
    RK: toBase64(rk3),
    CKs: toBase64(cks),
    CKr: toBase64(ckr),
...
async function decryptWithMK(mk: Uint8Array, ciphertext: string, iv: string): Promise<string> {
  ...
  try {
    ...
    throw new Error('MAC verification or bad message key');
  }
}
...
export async function ratchetEncrypt(
  ...
  if (kemsPub(s)) {
    headerFields.kem_pub = kemsPub(s);
  }
  if (s.KEM_ct) {
  ...
  let nextHKs = s.HKs;
  if (s.HKs) {
    nextHKs = await stepHK(s.HKs);
  }
  let nextHK = s.HK;
  if (s.HK) {
    nextHK = await stepHK(s.HK);
  }

  return {
    envelope,
    updatedSession: { ...s, CKs: toBase64(newCKs), Ns: s.Ns + 1, HKs: nextHKs, HK: nextHK },
  };
}
...
export async function ratchetDecrypt(
  ...
  let prevChainLength: number | undefined;
  let kemPub: string | undefined;
  let kemCt: string | undefined;

  let resolved: PlaintextHeader | null = null;
  let usedHK: string | null = null;
  let usedSkipIndex: number = -1;

  if (envelope.encryptedHeader) {
    // 1. Try skipped HKs
    if (s.HKSKIPPED) {
      for (let i = 0; i < s.HKSKIPPED.length; i++) {
        resolved = await tryDecryptHeader(s.HKSKIPPED[i], envelope.encryptedHeader);
        if (resolved) {
          usedHK = s.HKSKIPPED[i];
          usedSkipIndex = i;
          break;
        }
      }
    ...
    // 2. Try current HKr with stepping
    let currentHKr = s.HKr;
    let stepsHKr = 0;
    const newSkippedHKs: string[] = [];
    if (!resolved && currentHKr) {
      while (stepsHKr < 50) {
        resolved = await tryDecryptHeader(currentHKr, envelope.encryptedHeader);
        if (resolved) {
          usedHK = currentHKr;
          break;
        }
        newSkippedHKs.push(currentHKr);
        currentHKr = await stepHK(currentHKr);
        stepsHKr++;
      }
    }

    // 3. Try NHKr with stepping (next epoch)
    let currentNHKr = s.NHKr;
    let stepsNHKr = 0;
    const newSkippedNHKs: string[] = [];
    if (!resolved && currentNHKr) {
      while (stepsNHKr < 50) {
        resolved = await tryDecryptHeader(currentNHKr, envelope.encryptedHeader);
        if (resolved) {
          usedHK = 'next_epoch';
          await stepHK(currentNHKr); // suppress unused variable warning
          break;
        }
        newSkippedNHKs.push(currentNHKr);
        currentNHKr = await stepHK(currentNHKr);
        stepsNHKr++;
      }
    }

    // 4. Try legacy HK with stepping
    let currentHK = s.HK;
    let stepsHK = 0;
    const newSkippedLegacyHKs: string[] = [];
    if (!resolved && currentHK) {
      while (stepsHK < 50) {
        resolved = await tryDecryptHeader(currentHK, envelope.encryptedHeader);
        if (resolved) {
          usedHK = currentHK;
          break;
        }
        newSkippedLegacyHKs.push(currentHK);
        currentHK = await stepHK(currentHK);
        stepsHK++;
      }
    }

    // 5. Try the header keys stored alongside any skipped-message entries
    // (covers out-of-order messages from a previous epoch).
    if (!resolved) {
      const skippedMksHKs = new Set<string>();
      for (const val of Object.values(s.MKSKIPPED)) {
        try {
          const parsed = JSON.parse(val as string);
          if (parsed?.hk) skippedMksHKs.add(parsed.hk);
        } catch { /* plain-string legacy entry — no HK */ }
      }
      for (const hk of skippedMksHKs) {
    ...
    if (resolved) {
      ...
      // Update Session based on the used HK
      if (usedHK === 'next_epoch') {
        s.HKSKIPPED = [...(s.HKSKIPPED || []), ...newSkippedNHKs];
        // After dhRatchetStep, s.HKr will be set to s.NHKr
        // But since we consumed `stepsNHKr` from the NEXT epoch, we must also step the future s.HKr.
        // We will apply this after the dhRatchetStep.
      } else if (usedHK && usedSkipIndex !== -1) {
        // We used an already skipped HKr, remove it from the pool
        s.HKSKIPPED!.splice(usedSkipIndex, 1);
      } else if (usedHK === currentHKr && currentHKr) {
        // We stepped the current HKr
        s.HKSKIPPED = [...(s.HKSKIPPED || []), ...newSkippedHKs];
        s.HKr = await stepHK(currentHKr);
      } else if (usedHK === currentHK && currentHK) {
        // We stepped the legacy HK
        s.HKSKIPPED = [...(s.HKSKIPPED || []), ...newSkippedLegacyHKs];
        s.HK = await stepHK(currentHK);
      }
    } else if (envelope.header) {
      console.warn('Could not decrypt envelope header — falling back to cleartext header.');
      try {
        if (typeof envelope.header === 'string') {
          const h = JSON.parse(new TextDecoder().decode(fromBase64(envelope.header as string))) as PlaintextHeader;
      ...
      throw new Error('Could not decrypt envelope header — no matching header key.');
    }
  } else if (envelope.header) {
  ...
  if (senderPublicKey === undefined || messageNumber === undefined || prevChainLength === undefined) {
     throw new Error('Could not extract required header properties from envelope');
  }
  ...
  } else if (messageNumber < s.Nr) {
    throw new Error(`Message key for ${messageNumber} expired or missing (current Nr: ${s.Nr})`);
  }
