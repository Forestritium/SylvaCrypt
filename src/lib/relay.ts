...
import { toBase64, computeFingerprint } from '@/lib/cryptoCore';
...
import { fetchPrekeyBundle, x3dhSenderSetup, initRatchetFromX3DH, createSealedSenderBox, openSealedSenderBox, x3dhReceiverSetupFull, consumeOPKPrivate } from '@/lib/cryptoCore';
import { getEncrypted } from './localStore';
import { getDeviceKeyPair } from './deviceStore';
import { saveMessageToDBFull, updateContactPublicKey, updateMessageContentInDB, markMessageDeletedForEveryoneInDB, getMessagesFromDB } from './dbStore';
...
export async function uploadChatImage(
  userId: string,
  file: File
): Promise<{ storagePath: string; imageKeyBase64: string; imageMimeType: string }> {
  ...
  return { storagePath, imageKeyBase64: keyBase64, imageMimeType: file.type };
}
 ...
 * Pass the original mimeType so animated GIFs are reconstructed with the correct MIME
 * and play back normally in <img>. The caller is responsible for calling
...
export async function fetchAndDecryptChatImage(
  ...
  // Preserve the original MIME type so the browser renders animated GIFs correctly.
  // Falling back to 'image/jpeg' keeps existing behaviour for messages without MIME info.
  const resolvedMime = mimeType || 'image/jpeg';
  return URL.createObjectURL(new Blob([plainbuf], { type: resolvedMime }));
}
...
const senderBroadcastChannels = new Map<string, {
  channel: ReturnType<typeof supabase.channel>;
  pending: any[];
  subscribed: boolean;
}>();

function getSenderBroadcastChannel(userId: string) {
  const key = `user-broadcast:${userId}`;
  let entry = senderBroadcastChannels.get(key);
  if (!entry) {
    const channel = supabase.channel(key, { config: { broadcast: { ack: false } } });
    const pending: any[] = [];
    channel.subscribe((status) => {
      ...
      if (status === 'SUBSCRIBED' && pending.length > 0) {
        ...
        toFlush.forEach(p => channel.send({ type: 'broadcast', event: 'relay_message', payload: p }));
      }
    });
    entry = { channel, pending, subscribed: false };
    senderBroadcastChannels.set(key, entry);
  }
  return entry;
}

function broadcastRelayMessage(recipientId: string, row: RelayMessage) {
  const entry = getSenderBroadcastChannel(recipientId);
  if (entry.subscribed) {
    entry.channel.send({ type: 'broadcast', event: 'relay_message', payload: row });
  } else {
...
export async function sendEncryptedMessage(
  ...
  messageId?: string,
  imageAttachment?: { storagePath: string; imageKeyBase64: string; imageMimeType?: string },
  ...
  if (fileAttachment) {
    ...
    ratchetPlaintext = JSON.stringify({
      ...
      im: imageAttachment.imageMimeType ?? '',
    });
  }
  ...
  if (relayRows.length > 0) {
    ...
    if (data && data.length > 0) {
      ...
      data.forEach(row => {
        broadcastRelayMessage(recipientId, row as unknown as RelayMessage);
      });
    }
  }
  ...
  const localMsg: LocalMessage = {
    ...
    imageMimeType: imageAttachment?.imageMimeType ?? null,
...
export async function sendEditedMessage(
  ...
  if (relayRows.length > 0) {
    const { data, error } = await supabase.from('relay_messages').insert(relayRows).select();
    if (error) throw new Error(`Edit relay batch insert failed: ${error.message}`);
    if (data && data.length > 0) {
      data.forEach(row => broadcastRelayMessage(recipientId, row as unknown as RelayMessage));
    }
  }
}
...
export async function sendDeleteForEveryone(
  ...
  // --- Generate new session reset signal function ---
  // The receiver calls this when all decryption attempts fail to notify the sender
  // that their outgoing Double Ratchet state is invalid and needs a full restart.
  // We attach `v: 7` so that the receiver handles it differently from regular data payloads.

export async function sendSessionResetSignal(
  senderId: string,
  recipientId: string,
  conversationId: string,
  recipientPublicKey: string
  ...
  // v: 7 designates a session reset signal
  const ratchetPlaintext = JSON.stringify({ v: 7, reason: 'decryption_failed' });
  ...
  for (const target of targets) {
    ...
    try {
      ...
      // Ignore errors when sending the reset signal itself
      console.warn(`[SylvaCrypt] Failed to encrypt session reset signal for ${sessionKey}`, err);
    }
  }

  if (relayRows.length > 0) {
    const { data, error } = await supabase.from('relay_messages').insert(relayRows).select();
    if (error) console.error(`[SylvaCrypt] Session reset relay batch insert failed: ${error.message}`);
    if (data && data.length > 0) {
      data.forEach(row => broadcastRelayMessage(recipientId, row as unknown as RelayMessage));
    }
  }
...
async function _buildRecoveredLocalMessage(
  ...
  let imageStoragePath: string | null = null, imageKeyBase64: string | null = null, imageMimeType: string | null = null;
  ...
  try {
    const p: { v?: number; t?: string; mid?: string; isp?: string; ik?: string; im?: string; vsp?: string; vk?: string; vd?: number; fsp?: string; fk?: string; fn?: string; fs?: number; ft?: string } = JSON.parse(decrypted);

    // v:7 — session reset signal: clear outgoing session and return null (no UI message)
    if (p.v === 7) {
      console.warn(`[SylvaCrypt] Received session reset signal from ${senderUsername}. Attempting to clear outgoing session.`);
      const lockSessionKey = relayMessage.sender_device_id
        ? `${conversationId}:${relayMessage.sender_device_id}`
        : conversationId;
      await deleteRatchetSession(lockSessionKey).catch(() => {});
      await deleteLastKnownSenderIdk(lockSessionKey).catch(() => {});
      return null;
    }

    // v:6 — delete-for-everyone: tombstone the original message and return a mutation
    // marker so the caller applies the deletion to the in-memory message list.
    // Without this guard the raw JSON would fall through and render as message content.
    if (p.v === 6 && p.mid) {
      await markMessageDeletedForEveryoneInDB(_myUserId, p.mid).catch(() => {});
      return {
        id: p.mid,
        conversationId,
    ...
    // v:5 — edit notification: update stored content and return a mutation marker.
    if (p.v === 5 && p.mid) {
      const editedAt = Date.now();
      await updateMessageContentInDB(_myUserId, p.mid, p.t ?? '', editedAt).catch(() => {});
      return {
        id: p.mid,
        conversationId,
        senderId: relayMessage.sender_id,
    ...
    if (p.v === 2) { content = p.t ?? ''; imageStoragePath = p.isp ?? null; imageKeyBase64 = p.ik ?? null; imageMimeType = p.im || null; }
  ...
  return {
    ...
    imageStoragePath, imageKeyBase64, imageMimeType, replyTo: null,
...
async function _receiveAndDecryptMessageInner(
  ...
  try {
    ...
    // --- Process decrypted metadata without persisting session yet ---
    let _recovered: LocalMessage | null = null;
    let isResetSignalVar = false;
    if (decrypted) {
      const rec = await _buildRecoveredLocalMessage(relayMessage, conversationId, myUserId, senderUsername, payloadMid, decrypted);
      if (!rec) {
         isResetSignalVar = true; // Signals v:7 handled and discarded internally
         void isResetSignalVar;
      } else {
         _recovered = rec;
         void _recovered;
      }
    ...
    if (!decrypted && !ratchetSession) {
      // Temporarily allow fallback for recovery
      // throw new Error('Hard failure triggered to prevent downgrade attack. Legacy fallback disabled.');
      console.warn('Hard failure suppressed to allow legacy fallback during migration.');
    }
    ...
    if (!decrypted && ratchetSession) {
      try {
        ...
        console.warn('Failed ratchet decrypt with legacy session', err);
      }
    }
    ...
    if (relayMessage.sender_cert) {
      try {
        ...
        // pass server timestamp if available, for clock-skew resilient freshness check
        const serverTs = relayMessage.created_at ? new Date(relayMessage.created_at).getTime() : undefined;
        const cert = await openSealedSenderBox(box, myKP.privateKeyBase64, 60 * 60 * 1000, serverTs);
        if (cert) {
    ...
    try {
      const parsed: {
        ...
        isp?: string; ik?: string; im?: string;
      ...
      if (parsed.v === 5 && parsed.mid) {
        ...
        // Return a synthetic LocalMessage carrying the mutation flag
        return {
          id: parsed.mid,
          conversationId,
          senderId: relayMessage.sender_id,
      ...
      if (parsed.v === 2) {
        content = parsed.t ?? '';
        imageStoragePath = parsed.isp ?? null; imageKeyBase64 = parsed.ik ?? null; imageMimeType = parsed.im || null;
      } else if (parsed.v === 3) {
        content = parsed.t ?? '';
    ...
    const localMsg: LocalMessage = {
      ...
      imageUrl: null,
      imageStoragePath, imageKeyBase64, imageMimeType,
      ...
      if (myKP0) {
        try {
          ...
          if (spk0 && spk0.id === payloadX3DH.spk_id) {
            ...
            if (!recoveredX3DH) {
              console.info(`[SylvaCrypt] Handled session reset signal via X3DH retry for relay ${relayMessage.id}`);
              return null;
            }
            void (async () => {
    ...
    for (const tryKey of keysToTry) {
      ...
      try {
        ...
        if (!recovered) {
          console.info(`[SylvaCrypt] Handled session reset signal via legacy DH for relay ${relayMessage.id}`);
          return null;
        }
        void (async () => {
    ...
    console.error(`[SylvaCrypt] All decryption attempts exhausted — clearing stale session for conversation ${conversationId}`);
    
    // Attempt to delete stale receiver session to trigger a fresh X3DH setup upon next attempt
    try {
      const lockSessionKey = relayMessage.sender_device_id
        ? `${relayMessage.conversation_id}:${relayMessage.sender_device_id}`
        : relayMessage.conversation_id;
        
      await deleteRatchetSession(lockSessionKey);
      await deleteLastKnownSenderIdk(lockSessionKey);
      
      // Tell the sender that their session is broken and they must reset
      void sendSessionResetSignal(myUserId, relayMessage.sender_id, relayMessage.conversation_id, senderPublicKey).catch(e => {
        console.warn('[SylvaCrypt] Failed to send session reset signal:', e);
      });
    } catch (sessionErr) {
      console.warn('[SylvaCrypt] Failed to delete exhausted ratchet session:', sessionErr);
    }
    
    console.debug('[SylvaCrypt] Discarding undecryptable relay message', relayMessage.id);
    try {
      const { error: delErr } = await supabase
        .from('relay_messages')
        .delete()
      ...
      // Prevent redundant placeholders
      // Retrieve the messages directly to check if it's already recorded
      const msgs = await getMessagesFromDB(myUserId, conversationId);
      const existingPlaceholder = msgs.messages.find(m => m.id === relayMessage.id);
...
let sharedRelayChannelPg: ReturnType<typeof supabase.channel> | null = null;
let sharedRelayChannelBc: ReturnType<typeof supabase.channel> | null = null;
const relayListeners = new Set<RelayListener>();

export function clearRelayChannel(): void {
  if (sharedRelayChannelPg) {
    supabase.removeChannel(sharedRelayChannelPg);
    sharedRelayChannelPg = null;
  }
  if (sharedRelayChannelBc) {
    supabase.removeChannel(sharedRelayChannelBc);
    sharedRelayChannelBc = null;
  }
...
export function subscribeToRelay(
  ...
  function createChannel() {
    ...
    sharedRelayChannelPg = ch;

    // Create the broadcast channel for faster delivery
    if (!sharedRelayChannelBc) {
      sharedRelayChannelBc = supabase
        .channel(`user-broadcast:${userId}`, { config: { broadcast: { ack: false } } })
        .on('broadcast', { event: 'relay_message' }, ({ payload }) => {
          relayListeners.forEach(({ onMessage: cb }) => {
            try {
              const maybePromise = cb(payload as RelayMessage);
              if (maybePromise) {
                maybePromise.catch(err =>
                  console.error('[SylvaCrypt] Relay realtime broadcast handler error:', err)
                );
              }
            } catch (err) {
              console.error('[SylvaCrypt] Relay realtime broadcast handler error:', err);
  ...
  if (!sharedRelayChannelPg) {
    ...
    if ((sharedRelayChannelPg as unknown as { state: string }).state === 'joined') {
      if (onReady) onReady();
    }
  }
...
// --- Contact-request client-side rate limit ---
// Allows at most MAX_REQUESTS attempts per WINDOW_MS per receiver.
// This is a best-effort guard complementing Supabase RLS limits.
const CONTACT_REQUEST_MAX = 3;
const CONTACT_REQUEST_WINDOW_MS = 10 * 60 * 1000; // 10 minutes

/** timestamp → receiverId map stored as a simple ring buffer */
const _contactRequestLog: { receiverId: string; ts: number }[] = [];

function _checkContactRequestLimit(receiverId: string): void {
  const now = Date.now();
  // Drop entries outside the window
  while (_contactRequestLog.length > 0 && _contactRequestLog[0].ts < now - CONTACT_REQUEST_WINDOW_MS) {
    _contactRequestLog.shift();
  }
  const recentToReceiver = _contactRequestLog.filter(e => e.receiverId === receiverId).length;
  if (recentToReceiver >= CONTACT_REQUEST_MAX) {
    const oldest = _contactRequestLog.find(e => e.receiverId === receiverId)!;
    const resetInMs = CONTACT_REQUEST_WINDOW_MS - (now - oldest.ts);
    const resetInMin = Math.ceil(resetInMs / 60_000);
    throw new ContactRequestLimitError(resetInMin);
  }
  _contactRequestLog.push({ receiverId, ts: now });
}

/** Thrown when the client-side contact request rate limit is exceeded. */
export class ContactRequestLimitError extends Error {
  resetInMin: number;
  constructor(resetInMin: number) {
    super(`Too many contact requests. Please wait ${resetInMin} minute${resetInMin !== 1 ? 's' : ''} before trying again.`);
    this.name = 'ContactRequestLimitError';
    this.resetInMin = resetInMin;
  }
}
...
export async function sendContactRequest(
  ...
  // Client-side rate limit — max 3 requests per receiver per 10 minutes
  try {
    _checkContactRequestLimit(receiverId);
  } catch (e) {
    if (e instanceof ContactRequestLimitError) return { error: e.message };
    throw e;
  }
