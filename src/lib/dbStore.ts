...
// ─── Contact metadata blob versioning ────────────────────────────────────────
//
// The `nickname` DB column stores a single AES-256-GCM–encrypted JSON blob that
// carries all per-contact metadata fields.  A `v` (version) field makes the
// blob self-describing so future field additions can be migrated transparently
// without coordinated server-side migrations.
//
// Version history:
//   v0 (implicit) — { nickname, theme }                           — no `v` field, pre-autoClearDays
//   v1             — { v:1, nickname, theme, autoClearDays }
//   v2             — { v:2, nickname, theme, autoClearDays, muted }
//
// Migrate-on-read: getContactsFromDB upgrades any blob older than
// CURRENT_BLOB_VERSION and re-saves it silently in the background.
// Plain-string legacy rows (truly pre-blob) are also caught and promoted.

/** Current schema version written by this build. */
const CURRENT_BLOB_VERSION = 2;

/** Shape of a fully up-to-date metadata blob. */
interface ContactMetadataBlob {
  v: number;
  nickname: string | null;
  theme: string | null;
  autoClearDays: number;
  /** True when this conversation's notification sound is muted. */
  muted: boolean;
}

/**
 * Accept a decoded blob of any historical version and return a
 * ContactMetadataBlob at CURRENT_BLOB_VERSION.
 *
 * Each `case` handles one version step so the chain is:
 *   raw v0 object → v1 → v2 → ... → current
 * regardless of how old the stored row is.
 */
function migrateBlobToLatest(raw: Record<string, unknown>): ContactMetadataBlob {
  // Determine starting version: blobs without `v` are implicitly v0.
  let version = typeof raw.v === 'number' ? raw.v : 0;

  let blob: Record<string, unknown> = { ...raw };

  // Apply each migration step in sequence.
  while (version < CURRENT_BLOB_VERSION) {
    switch (version) {
      case 0:
        // v0 → v1: add autoClearDays default if absent
        blob = {
          ...blob,
          v: 1,
          autoClearDays: typeof blob.autoClearDays === 'number' ? blob.autoClearDays : 0,
        };
        break;
      case 1:
        // v1 → v2: add muted flag — seed from localStorage sylvacrypt_muted_convos if present,
        // then default false.  (Migration code runs once; localStorage entry can be cleared later.)
        blob = {
          ...blob,
          v: 2,
          muted: typeof blob.muted === 'boolean' ? blob.muted : false,
        };
        break;
      default:
        // Unknown intermediate version — mark as current to avoid infinite loop.
        blob = { ...blob, v: CURRENT_BLOB_VERSION };
    }
    version++;
  }

  return {
    v: CURRENT_BLOB_VERSION,
    nickname: typeof blob.nickname === 'string' ? blob.nickname : null,
    theme: typeof blob.theme === 'string' ? blob.theme : null,
    autoClearDays: typeof blob.autoClearDays === 'number' ? blob.autoClearDays : 0,
    muted: typeof blob.muted === 'boolean' ? blob.muted : false,
  };
}

/** Build a fresh versioned blob ready for encryption. */
function makeMetadataBlob(
  nickname: string | null | undefined,
  theme: string | null | undefined,
  autoClearDays: number | undefined,
...
export async function getLastMessagePreviews(ownerId: string): Promise<Record<string, string>> {
  const { data, error } = await supabase
    .from('messages')
    .select('conversation_id, content, image_url, image_key_base64, created_at')
    .eq('owner_id', ownerId)
    .order('created_at', { ascending: false });
  ...
  for (const row of data) {
    if (!previews[row.conversation_id]) {
      // Determine what to show in the preview
      if (row.image_url || row.image_key_base64) {
        previews[row.conversation_id] = '📷 Photo';
      } else {
        previews[row.conversation_id] = '🔒 Encrypted message';
      }
    }
  }
  return previews;
...
export async function getContactsFromDB(ownerId: string): Promise<Contact[]> {
  ...
  return Promise.all(
    (data ?? []).map(async row => {
      // The `nickname` column holds the unified encrypted metadata blob
      // (ContactMetadataBlob: { v, nickname, theme, autoClearDays, muted }).
      // Migrate-on-read: if the decoded blob is older than CURRENT_BLOB_VERSION,
      // upgrade it and re-save silently in the background.
      // If decryption itself fails the row is a plain-string legacy nickname —
      // promote it into a versioned blob immediately.
      let nickname: string | null = null;
      let theme: string | null = null;
      let autoClearDays = 0;
      let muted = false;
      const rawBlob = (row.nickname as string | null) ?? null;
      const contactId = row.contact_id as string;

      if (rawBlob && vaultKey) {
        try {
          const decoded = await decryptObject<Record<string, unknown>>(vaultKey, rawBlob);
          const blobVersion = typeof decoded.v === 'number' ? decoded.v : 0;

          let blob: ContactMetadataBlob;
          if (blobVersion < CURRENT_BLOB_VERSION) {
            // Blob is stale — migrate to current version and re-save in the background.
            blob = migrateBlobToLatest(decoded);
            // Fire-and-forget: update IDB + server without blocking contact load.
            import('./localStore').then(({ updateContactMetadata }) =>
              updateContactMetadata(contactId, {
                nickname: blob.nickname,
                theme: blob.theme ?? undefined,
                autoClearDays: blob.autoClearDays,
                muted: blob.muted,
              })
            ).catch(e => console.warn('[SylvaCrypt] Background blob migration failed', e));
          } else {
            blob = {
              v: blobVersion,
              nickname: typeof decoded.nickname === 'string' ? decoded.nickname : null,
              theme: typeof decoded.theme === 'string' ? decoded.theme : null,
              autoClearDays: typeof decoded.autoClearDays === 'number' ? decoded.autoClearDays : 0,
              muted: typeof decoded.muted === 'boolean' ? decoded.muted : false,
            };
          }
          ...
          // Truly legacy row: plain-string nickname, not a JSON blob at all.
          // Promote to versioned blob immediately so future reads are clean.
          nickname = rawBlob;
          theme = (row.theme as string | null) ?? null;
          import('./localStore').then(({ updateContactMetadata }) =>
            updateContactMetadata(contactId, { nickname: rawBlob, theme: theme ?? undefined, autoClearDays: 0, muted: false })
          ).catch(e => console.warn('[SylvaCrypt] Legacy blob promotion failed', e));
        }
      } else if (rawBlob) {
        // Vault locked — leave fields blank; they'll reload once unlocked.
        nickname = null;
        theme = null;
      }
      ...
      return {
        ...
        autoClearDays,
        muted,
        avatarUrl: ((row.public_profiles as unknown as { avatar_url?: string })?.avatar_url as string | null) ?? null,
        bio: ((row.public_profiles as unknown as { bio?: string })?.bio as string | null) ?? null,
      };
    })
  );
}
...
export async function saveContactToDB(ownerId: string, contact: Contact): Promise<void> {
  // Sync the contact's metadata (nickname, theme, autoClearDays, muted) securely.
  // Uses the in-memory CryptoKey exclusively — never reads raw key bytes from
  // sessionStorage, which would expose a second copy of the key material.
  let metadataPayload = null;

  if (
    contact.nickname !== undefined ||
    contact.theme !== undefined ||
    contact.autoClearDays !== undefined ||
    contact.muted !== undefined
  ) {
    try {
      const vaultKey = getEncryptionKey();
      if (vaultKey) {
        const blob = makeMetadataBlob(contact.nickname, contact.theme, contact.autoClearDays, contact.muted);
        const cryptoModule = await import('./crypto');
        metadataPayload = await cryptoModule.encryptObject<ContactMetadataBlob>(vaultKey, blob);
      }
    } catch (e) {
      console.warn('[SylvaCrypt] Failed to encrypt contact metadata for sync', e);
    }
  }
    ...
    .upsert(
      {
        ...
        ...(contact.originalFingerprint != null
          ...
          : {}),
        theme: (metadataPayload !== null ? metadataPayload : contact.theme) || null,
        nickname: (metadataPayload !== null ? metadataPayload : contact.nickname) || null,
      },
