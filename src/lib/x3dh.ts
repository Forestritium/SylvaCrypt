...
export async function openSealedSenderBox(
  ...
  try {
    ...
    // Verify cert freshness against server time if provided, otherwise client time
    // If the sealed sender box has a difference of more than maxAgeMs, we reject it.
    // The previous clock sync issues caused this to throw false positives for clients with misconfigured system clocks.
    // Let's use a slightly larger max age (60 minutes) or fallback to server time entirely.
    const referenceTime = serverTsMs ?? Date.now();
    
    // We allow a maximum skew of 60 minutes (3600000 ms) instead of 5 minutes 
    // to handle legitimate device sleep or background syncing edge cases
    const MAX_SKEW_MS = 60 * 60 * 1000; 
    
    if (Math.abs(referenceTime - cert.ts) > Math.max(maxAgeMs, MAX_SKEW_MS)) {
      console.warn(`[X3DH] Sealed sender cert is stale, rejecting. Cert TS: ${cert.ts}, Ref TS: ${referenceTime}`);
      return null;
    }
