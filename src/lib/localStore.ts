...
import type { Contact, LocalMessage, RatchetSession } from '@/types/types';
...
export async function setEncryptionKey(key: CryptoKey): Promise<void> {
  ...
  try {
    ...
    // Seed the sessionStorage caches used by the synchronous isDND() and
    // isMuted() hot-paths in notificationSound.ts from the encrypted store.
    _seedNotificationCaches().catch(() => {});
...
export async function restoreVaultKey(): Promise<boolean> {
  ...
  try {
    ...
    encryptionKey = key;
    // Seed the sessionStorage caches used by the synchronous isDND() and
    // isMuted() hot-paths now that the vault key is available.
    _seedNotificationCaches().catch(() => {});
    return true;
  } catch {
    return false;
...
/**
 * Seed the sessionStorage write-through caches used by the synchronous
 * isDND() and isMuted() hot-paths in notificationSound.ts.
 *
 * Called after the vault key becomes available (setEncryptionKey /
 * restoreVaultKey) so the caches are accurate from the first message arrival.
 * Intentionally fire-and-forget — a missing cache means the synchronous
 * fallback returns false (silent / unmuted), which is the safe default.
 *
 * @internal — not exported; only called from setEncryptionKey / restoreVaultKey.
 */
async function _seedNotificationCaches(): Promise<void> {
  try {
    // Seed DND cache.
    const prefs = await getUserPrefs();
    if (prefs.dnd) sessionStorage.setItem('sc_dnd_cache', '1');
    else sessionStorage.removeItem('sc_dnd_cache');
    // Seed typing-disabled cache.
    if (prefs.typingDisabled) sessionStorage.setItem('sc_typing_disabled_cache', '1');
    else sessionStorage.removeItem('sc_typing_disabled_cache');
    // Seed waveform cache.
    sessionStorage.setItem('sc_waveform_cache', JSON.stringify({ type: prefs.waveformType, color: prefs.waveformColor }));
  } catch { /* non-fatal */ }

  try {
    // Seed muted-convos cache from encrypted contact metadata blobs.
    const contacts = await getContacts();
    const mutedIds = contacts
      .filter(c => c.muted)
      .map(c => c.conversationId);
    sessionStorage.setItem('sc_muted_cache', JSON.stringify(mutedIds));
  } catch { /* non-fatal */ }
}
...
export async function updateContactMetadata(
  contactId: string,
  metadata: { nickname?: string | null; theme?: string; autoClearDays?: number; muted?: boolean },
): Promise<void> {
  const contacts = await getContacts();
  const contact = contacts.find(c => c.id === contactId);
  if (!contact) return;

  if (metadata.nickname !== undefined) contact.nickname = metadata.nickname;
  if (metadata.theme !== undefined) contact.theme = metadata.theme;
  if (metadata.autoClearDays !== undefined) contact.autoClearDays = metadata.autoClearDays;
  if (metadata.muted !== undefined) contact.muted = metadata.muted;

  await saveContact(contact);

  // Sync metadata to server using the in-memory CryptoKey — never reads raw
  // key bytes from sessionStorage. Always writes a versioned blob (v field
  // included) so every write path produces a self-describing, migratable blob.
  // v is pinned to 2 here — keep in sync with CURRENT_BLOB_VERSION in dbStore.ts.
  try {
    ...
    const blob = {
      ...
      theme: contact.theme ?? null,
      autoClearDays: contact.autoClearDays ?? 0,
      muted: contact.muted ?? false,
    };

    const cryptoModule = await import('./crypto');
    const b64 = await cryptoModule.encryptObject(vaultKey, blob);
    ...
    if (session?.user) {
      await supabase
        .from('contacts')
        .update({ nickname: b64, theme: b64 })
        .eq('contact_id', contactId)
        .eq('owner_id', session.user.id);
    }
  } catch (e) {
    console.warn('[SylvaCrypt] Failed to sync contact metadata to server', e);
  }
}
...
export async function removeContactAndMessages(contactId: string, conversationId: string): Promise<void> {
  ...
  await Promise.all([
    db.delete('contacts', contactId),
    db.delete('messages', conversationId),
  ]);
}
...
// ========================
//
// User-level preferences (Do Not Disturb, etc.) that were previously stored as
// cleartext in localStorage are now stored in the vault-encrypted IDB store.
// This prevents an attacker with filesystem access from learning behavioural
// metadata about the user.
//
// Key: 'user_prefs'
// Shape: UserPrefs (see interface below)

interface UserPrefs {
  dnd: boolean;
  /** True when the user has disabled outgoing typing indicators. */
  typingDisabled: boolean;
  /** Waveform style/color preferences (previously sc_waveform_settings). */
  waveformType: string;
...
async function getUserPrefs(): Promise<UserPrefs> {
  const stored = await getEncrypted<Partial<UserPrefs>>(USER_PREFS_KEY);
  return stored ? { ...DEFAULT_USER_PREFS, ...stored } : { ...DEFAULT_USER_PREFS };
}

async function saveUserPrefs(prefs: UserPrefs): Promise<void> {
  await setEncrypted(USER_PREFS_KEY, prefs);
}

/**
 * Returns the current Do Not Disturb setting from the encrypted store.
 * Async — callers should await or use the synchronous localStorage shim in
 * notificationSound.ts for hot-path checks that run during message delivery.
 */
export async function getDND(): Promise<boolean> {
  return (await getUserPrefs()).dnd;
}

/** Persist the Do Not Disturb setting to the encrypted store. */
export async function setDNDPref(enabled: boolean): Promise<void> {
  const prefs = await getUserPrefs();
  prefs.dnd = enabled;
  await saveUserPrefs(prefs);
  // Keep a cleartext mirror only as a fast read-cache for the synchronous
  // isDND() hot path (message-arrival checks).  The authoritative value is
  // always the encrypted store; this mirror is written immediately after the
  // encrypted write so it is never stale by more than one operation.
  try {
    if (enabled) sessionStorage.setItem('sc_dnd_cache', '1');
    else sessionStorage.removeItem('sc_dnd_cache');
  } catch { /* ignore */ }
}

/**
 * Returns the typing-disabled preference from the encrypted store.
 * For the synchronous hot-path use getTypingDisabledSync() which reads
 * the sessionStorage write-through cache.
 */
export async function getTypingDisabledPref(): Promise<boolean> {
  return (await getUserPrefs()).typingDisabled;
}

/** Persist the typing-disabled flag to the encrypted store. */
export async function setTypingDisabledPref(disabled: boolean): Promise<void> {
  const prefs = await getUserPrefs();
  prefs.typingDisabled = disabled;
  await saveUserPrefs(prefs);
  // Write-through cache for synchronous reads in the message-send hot-path.
  try {
    if (disabled) sessionStorage.setItem('sc_typing_disabled_cache', '1');
    else sessionStorage.removeItem('sc_typing_disabled_cache');
  } catch { /* ignore */ }
}

/** Synchronous read of typing-disabled — reads the sessionStorage cache. */
export function getTypingDisabledSync(): boolean {
  try { return sessionStorage.getItem('sc_typing_disabled_cache') === '1'; } catch { return false; }
}

/** Returns the waveform settings from the encrypted store. */
export async function getWaveformSettingsPref(): Promise<{ type: string; color: string }> {
  const prefs = await getUserPrefs();
  return { type: prefs.waveformType, color: prefs.waveformColor };
}

/** Persist waveform settings to the encrypted store. */
export async function setWaveformSettingsPref(type: string, color: string): Promise<void> {
  const prefs = await getUserPrefs();
  prefs.waveformType = type;
  prefs.waveformColor = color;
  await saveUserPrefs(prefs);
  // Write-through cache for synchronous reads in useWaveformSettings.
  try {
    sessionStorage.setItem('sc_waveform_cache', JSON.stringify({ type, color }));
  } catch { /* ignore */ }
}
...
import { RATCHET_VERSION } from './doubleRatchet';

export async function getRatchetSession(
  conversationId: string
): Promise<RatchetSession | null> {
  const session = await getEncrypted<RatchetSession>(`ratchet_v3:${conversationId}`);
  if (session && session.version !== RATCHET_VERSION) {
...
/**
 * Find the ratchet session for a conversation.  Ratchet sessions may be stored
 * under the bare conversationId (legacy / single-device) or under
 * `conversationId:deviceId` when the contact has linked devices.  This helper
 * tries the bare key first, then scans the vault for any key with the same
 * conversation prefix.
 */
export async function getRatchetSessionForConversation(conversationId: string): Promise<RatchetSession | null> {
  const bare = await getRatchetSession(conversationId);
  if (bare) return bare;

  try {
    const database = await getDB();
    const allKeys = await database.getAllKeys('encrypted_store') as string[];
    const prefix = `ratchet_v3:${conversationId}:`;
    const matching = allKeys.filter(k => typeof k === 'string' && k.startsWith(prefix));
    for (const key of matching) {
      const session = await getEncrypted<RatchetSession>(key);
      if (session) return session;
    }
  } catch (e) {
    console.warn('[SylvaCrypt] getRatchetSessionForConversation scan failed:', e);
  }
  return null;
}
