...
import {
  ...
  AlertCircle, Clock, ArrowLeft, Image as ImageIcon, X, Reply, Bell, BellOff, Paperclip, Check,
  Pencil, Trash2, MoreHorizontal, MoreVertical, ShieldAlert, ShieldCheck,
  Pin, PinOff, Eye, EyeOff, Timer, Palette, UserPen, CheckCircle2, Search, BookOpen
} from 'lucide-react';
...
import { EmojiReactionPicker, EmojiPickerPanel } from './EmojiReactionPicker';
import { playNotificationSound, unlockAudio, isMuted, setMuted, isDND } from '@/lib/notificationSound';
import { updateContactMetadata, getTypingDisabledSync } from '@/lib/localStore';
...
import { PerContactThemeDialog } from './PerContactThemeDialog';
...
interface ChatAreaProps {
  ...
  /** Called when local contact metadata (nickname/theme) is updated so the parent can refresh contacts. */
...
function MessageBubble({
  message, isSelf, showAvatar, senderInitial, senderAvatarUrl, onReply, onScrollTo, decryptedImageUrl, onReact, currentUserId,
  ...
  // Mobile long-press → emoji reaction picker
  const [showMobileReactionPicker, setShowMobileReactionPicker] = useState(false);
  // Swipe-right to reply (touch) + long-press detection
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    ...
    if (!message.isViewOnce) {
      longPressTimerRef.current = setTimeout(() => {
        ...
        setShowMobileReactionPicker(true);
      }, LONG_PRESS_MS);
    }
  };
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    // Cancel long-press if the user is scrolling/swiping
  ...
  return (
    <>
    {/* Mobile long-press emoji reaction overlay — panel opens directly, no trigger button */}
    {showMobileReactionPicker && (
      <div
        className="fixed inset-0 z-50 flex items-end justify-center pb-8 md:hidden"
        onClick={() => setShowMobileReactionPicker(false)}
      >
        <div
        ...
        >
          <EmojiPickerPanel
            onSelect={emoji => { onReact(message, emoji); setShowMobileReactionPicker(false); }}
          />
        </div>
      </div>
    )}
    <div
      className={`flex w-full items-end gap-2 group select-none md:select-text ${isSelf ? 'flex-row-reverse' : 'flex-row'}`}
    ...
    >
      {!isSelf && (
        <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 self-end mb-1 overflow-hidden ${
          showAvatar ? 'bg-muted border border-border' : 'invisible'
        }`}>
          {senderAvatarUrl ? (
            <img src={senderAvatarUrl} alt={senderInitial} className="w-full h-full object-cover" />
          ) : (
      ...
      {/* Reply button — desktop hover only; hidden on mobile (use swipe-to-reply instead) */}
      {!message.isViewOnce && (
        <button
          type="button"
          aria-label="Reply"
          onClick={() => onReply(message)}
          className={`hidden md:flex shrink-0 self-center w-7 h-7 items-center justify-center rounded-full text-muted-foreground hover:text-primary hover:bg-muted transition-all duration-150 ${
      ...
      {/* Emoji reaction picker — desktop hover only; mobile uses long-press overlay */}
      {!message.isViewOnce && (
        <div className={`hidden md:block shrink-0 self-center transition-all duration-150 ${
          showReplyBtn ? 'opacity-100 scale-100' : 'opacity-0 scale-75 pointer-events-none'
        } order-last`}>
      ...
      <div className={`min-w-0 max-w-[75%] flex flex-col gap-0.5 ${isSelf ? 'items-end' : 'items-start'}`}>
        ...
        {message.isDeletedForEveryone ? (
          ...
          <div className={`px-3 py-2 rounded-2xl text-sm leading-relaxed italic text-muted-foreground border border-dashed border-border/60 rounded-br-sm`}>
            <EyeOff className="w-3.5 h-3.5 inline mr-1.5" />
            Sent as view once message
          </div>
        ) : message.isViewOnce && !isSelf ? (
          <button
            type="button"
            ...
            <div className="mb-1">
              ...
              {imgOpen && (
                <div
                  ...
                  onKeyDown={e => { if (e.key === 'Escape') setImgOpen(false); }}
          ...
          {!message.imageUrl && decryptedImageUrl && (
            <div className="mb-1">
              <img
                ...
                alt="Shared image"
                className={`rounded-xl max-w-[220px] md:max-w-[280px] w-full cursor-pointer hover:opacity-90 transition-opacity ${message.imageMimeType === 'image/gif' ? 'object-contain' : 'object-cover'}`}
                onClick={() => setImgOpen(true)}
                draggable={false}
              />
              {imgOpen && (
...
export function ChatArea({
  ...
  // Auto-clear chat dialog
  const [autoClearDialogOpen, setAutoClearDialogOpen] = useState(false);
  const [autoClearDays, setAutoClearDays] = useState<number>(0);
  // Search-in-conversation
  const [msgSearchOpen, setMsgSearchOpen] = useState(false);
  const [msgSearchQuery, setMsgSearchQuery] = useState('');
  // Per-contact theme dialog
  ...
  const { setContactTheme } = useTheme();

  // Apply or clear the per-contact theme whenever the active conversation changes.
  useEffect(() => {
    if (!conversation?.contact?.id) {
      setContactTheme(null);
      return;
    }
    // Only search the contacts array when the conversation ID changes,
    // avoiding unnecessary re-renders when contacts array is mutated elsewhere
    const contact = contacts.find(c => c.id === conversation.contact?.id);
    if (contact?.theme) {
      setContactTheme(contact.theme);
    } else {
      setContactTheme(null);
    }
    // We intentionally omit `contacts` from the dependency array because if a contact's
    // theme changes, that will happen in the handleThemeChange callback, and we don't
    // want other contact mutations (like unread counts) to cause a re-evaluation of the theme.
  }, [conversation?.contact?.id, setContactTheme]);

  // Clear any per-contact theme when this chat area unmounts.
  useEffect(() => {
    return () => setContactTheme(null);
  }, [setContactTheme]);
  ...
  // Global keyboard shortcut: open the shared notebook panel.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    (async () => {
      await Promise.allSettled(
        pending.map(async m => {
          try {
            const blobUrl = await fetchAndDecryptChatImage(m.imageStoragePath!, m.imageKeyBase64!, m.imageMimeType);
  ...
  useEffect(() => {
    ...
    setContactBio(null);
    // Load mute preference for this conversation from the contact's vault-encrypted
    // metadata blob (contactObj.muted), falling back to the sessionStorage cache
    // for the brief window before contacts are loaded.
    const contactObjForMute = contacts.find(c => c.conversationId === conversation.id);
    setMutedState(contactObjForMute?.muted ?? isMuted(conversation.id));
    ...
    // Load auto-clear days from vault-encrypted contact metadata.
    // Falls back to 0 (disabled) when the contact is not found or the field is absent.
    // localStorage is intentionally NOT used — sc_auto_clear was removed to avoid
    // storing conversation settings in cleartext.
    const contactForConv = contacts.find(c => c.id === conversation.contact?.id);
    setAutoClearDays(contactForConv?.autoClearDays ?? 0);
    ...
    if (conversation.type === 'direct' && conversation.contact?.id) {
      supabase
        ...
        .then(({ data }) => {
          ...
          const isAvatarPrivate = data?.avatar_private === true || (Array.isArray(data?.avatar_private) && !data.avatar_private.includes(currentUserId));
          setContactAvatarPrivate(isAvatarPrivate);
        });
    }
  ...
  // Typing indicator: respect the typing_disabled user preference
    if (conversation?.type === 'direct' && conversation.contact?.id) {
      if (!getTypingDisabledSync()) {
        if (typingTimeoutRef.current) return;
        // The broadcast is now async but we don't need to await it
  ...
  const handleCloseViewOnce = useCallback(async () => {
    ...
    try {
      ...
      setMessages(prev => prev.filter(m => m.id !== viewOnceMsg.id));
    } catch (err) {
  ...
  const handleSetAutoClear = useCallback((days: number) => {
    if (!conversation) return;
    setAutoClearDays(days);

    // Persist the setting in the vault-encrypted contact metadata blob.
    // This replaces the previous localStorage approach (sc_auto_clear:{id})
    // which stored the value in cleartext.
    const contactId = conversation.contact?.id;
    if (contactId) {
      updateContactMetadata(contactId, { autoClearDays: days }).catch(err =>
        console.warn('[SylvaCrypt] Failed to persist autoClearDays', err)
      );
    }

    if (days === 0) {
      toast.success('Auto clear disabled');
    } else {
      toast.success(`Messages will be auto-cleared after ${days} ${days === 1 ? 'day' : 'days'}`);

      // Apply immediately to messages already loaded in memory
      const msInDay = 24 * 60 * 60 * 1000;
      const cutoff = Date.now() - days * msInDay;
      setMessages(prev => prev.filter(m => m.timestamp >= cutoff));
    }
    setAutoClearDialogOpen(false);
  }, [conversation]);
  ...
  const handleSend = useCallback(async () => {
    ...
    let imageAttachment: { storagePath: string; imageKeyBase64: string; imageMimeType: string } | undefined;
    ...
    input, selectedImage, selectedFile, conversation, sending, uploadingImage, uploadingFile,
    contacts, activeKeyAlert, currentUserId, currentUsername, replyingTo, ttlSeconds, sendAsViewOnce,
    onMessageSent
  ]);
  ...
  const handleVoiceRecorded = useCallback(async (blob: Blob, durationSeconds: number, mimeType: string) => {
    ...
    const contact = contacts.find(c => c.id === conversation.contact?.id);
    if (!contact) { toast.error('Contact not found.'); return; }
    const currentTtlSeconds = ttlSeconds;
    ...
    const optimistic: LocalMessage = {
      ...
      fileSize: null,
      fileMimeType: null,
      ttlSeconds: currentTtlSeconds || null,
      expiresAt: currentTtlSeconds ? Date.now() + currentTtlSeconds * 1000 : null,
    };
    setMessages(prev => [...prev, optimistic].sort((a, b) => a.timestamp - b.timestamp));
  ...
  }, [conversation, contacts, currentUserId, currentUsername, uploadingVoice, onMessageSent, ttlSeconds]);
  ...
  const groupedMessages = messages.reduce<{ date: string; messages: LocalMessage[] }[]>((acc, msg) => {
    // Filter by search term when search is active
    if (msgSearchTerm && !msg.content?.toLowerCase().includes(msgSearchTerm)) return acc;
  ...
  // Update the parent whenever local contact metadata changes
  ...
  const displayName = contactObj?.nickname?.trim() ? contactObj.nickname.trim() : conversation.name;
  ...
  return (
    <div className="relative flex-1 flex flex-col min-w-0 overflow-hidden bg-background" onContextMenu={captureDeterrenceProps.onContextMenu}>
      ...
      <div className="shrink-0 flex items-center gap-3 px-4 py-3 border-b border-border bg-card">
        ...
        <div className="w-9 h-9 rounded-full bg-muted border border-border flex items-center justify-center shrink-0 overflow-hidden">
          {contactAvatarUrl && !contactAvatarPrivate ? (
            ...
            />
          ) : contactObj?.avatarUrl && !contactAvatarPrivate ? (
             <img
              src={contactObj.avatarUrl}
              alt={displayName}
              className="w-full h-full object-cover"
            />
          ) : (
            <span className="text-sm font-semibold text-foreground">{displayName.charAt(0).toUpperCase()}</span>
          )}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-foreground truncate">
            {contactObj?.nickname?.trim() ? (
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-48">
            ...
            {contactObj && (
              <DropdownMenuItem
                onClick={() => {
                  setNicknameInput(contactObj.nickname ?? '');
            ...
            <DropdownMenuItem
              onClick={() => setAutoClearDialogOpen(true)}
              className="gap-2 cursor-pointer"
            >
        ...
        <div className="flex items-center gap-2 px-3 py-2 border-b border-border bg-background shrink-0">
          ...
          {msgSearchQuery && (
            <span className="text-xs text-muted-foreground shrink-0">
              {groupedMessages.reduce((n, g) => n + g.messages.length, 0)} result{groupedMessages.reduce((n, g) => n + g.messages.length, 0) !== 1 ? 's' : ''}
            </span>
          )}
          <button
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden min-h-0 px-4 py-4 space-y-4">
        ...
        ) : msgSearchTerm && groupedMessages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-3 text-center">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">No messages match "{msgSearchQuery}"</p>
            </div>
          </div>
        ) : (
            ...
            <div key={group.date} className="w-full space-y-1.5">
              ...
              <div className="w-full space-y-1">
                {group.messages.map((msg, idx) => {
                  ...
                  const senderAvatarUrl = (conversation.contact as any)?.avatarUrl ?? null;
      ...
      <div className="shrink-0 border-t border-border bg-card px-4 py-3">
        ...
        {!editingMessage && remoteTyping && !isRequestPending && (
          <TypingIndicator name={`@${displayName}`} visible={remoteTyping} />
        )}
        ...
        {isRequestPending ? (
          <div className="flex items-center gap-2.5 bg-amber-500/10 border border-amber-500/25 rounded-xl px-4 py-3">
            ...
            <p className="text-sm text-amber-600 dark:text-amber-400 text-pretty">
              {isOutgoingPending
                ? `Waiting for @${displayName} to accept your contact request.`
                : `Accept @${displayName}'s request in the sidebar to start messaging.`}
            </p>
          </div>
        ) : (
              ...
              <div className="flex items-start gap-2 bg-muted/60 border border-border rounded-xl px-3 py-2">
                <div className="relative shrink-0">
                  <img
                    src={imagePreview}
                    alt="Selected"
                    className={`w-16 h-16 rounded-lg border border-border ${selectedImage?.type === 'image/gif' ? 'object-contain' : 'object-cover'}`}
                  />
                  <button
            ...
            <div className="flex items-end gap-2">
              ...
              {isRecording ? (
                ...
                <Textarea
                  ...
                  className="message-input-box flex-1 min-w-0 border border-border text-foreground placeholder:text-muted-foreground resize-none min-h-[44px] max-h-32 px-3 py-2.5 text-sm leading-relaxed rounded-xl"
                  rows={1}
                />
              )}
            ...
            {Array.from(conversationPins.keys()).map(id => messages.find(m => m.id === id)).filter(Boolean).map(msg => (
              <div key={msg!.id} className="p-3 rounded-xl bg-muted/40 border border-border/60 text-sm">
                ...
                <button
                  ...
                  onClick={() => { setPinPanelOpen(false); scrollToMessage(msg!.id); }}
                >
      ...
      <Dialog open={!!viewOnceMsg} onOpenChange={(open) => { if (!open) handleCloseViewOnce(); }}>
        <DialogContent className="max-w-[calc(100%-2rem)] md:max-w-3xl bg-black/90 border-border max-h-[90dvh] flex flex-col p-4">
          <DialogHeader className="sr-only">
            ...
            <DialogDescription>This message can only be viewed once and will be deleted after closing.</DialogDescription>
          </DialogHeader>
      ...
      {/* Per-contact theme picker */}
      {contactObj && (
        <PerContactThemeDialog
          open={themeDialogOpen}
          onOpenChange={setThemeDialogOpen}
          currentThemeId={contactObj.theme ?? null}
          onSelect={async themeId => {
            if (!contactObj || !currentUserId) return;
            const updated = { ...contactObj, theme: themeId };
            await saveContactToDB(currentUserId, updated);
            
            // Immediately apply the new theme to context so it updates without remounting ChatArea
            setContactTheme(themeId);
            
            // Tell parent to refresh DB models
            handleContactsChange();
            
            setThemeDialogOpen(false);
            toast.success(themeId ? 'Per-contact theme saved.' : 'Using default theme for this contact.');
          }}
        />
      )}
      ...
      <Dialog open={nicknameDialogOpen} onOpenChange={setNicknameDialogOpen}>
        <DialogContent className="max-w-[calc(100%-2rem)] md:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              This name is only visible to you and will be shown instead of @{conversation.name}.
            </DialogDescription>
          </DialogHeader>
          <div className="py-2">
            <Input
          ...
          <DialogFooter>
            ...
            <Button
              onClick={async () => {
                ...
                const updated = { ...contactObj, nickname: trimmed || null };
                await saveContactToDB(currentUserId, updated);
                handleContactsChange();
      ...
      {ttlDialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
          <div className="bg-card w-full max-w-sm rounded-xl shadow-lg border border-border overflow-hidden">
            <div className="p-4 border-b border-border">
              ...
              <p className="text-sm text-muted-foreground mt-1 text-pretty">
                New messages will disappear from this chat for everyone after the selected time.
              </p>
            </div>
              ...
              {[
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setTtlDialogOpen(false);
                    // Don't auto-focus input on mobile as it pops keyboard over the UI we just closed
                ...
                >
                  {opt.label}
                  {ttlSeconds === opt.value && <CheckCircle2 className="w-4 h-4" />}
                </button>
              ))}
            </div>
      ...
      <Dialog open={autoClearDialogOpen} onOpenChange={setAutoClearDialogOpen}>
        <DialogContent className="max-w-[calc(100%-2rem)] md:max-w-md w-full">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground text-pretty">
              Automatically delete messages in this chat after a certain time.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2 mt-2">
            {[
              ...
              <button
                key={option.value}
                onClick={() => handleSetAutoClear(option.value)}
                className={`w-full px-4 py-3 rounded-xl flex items-center justify-between transition-colors
                  ${autoClearDays === option.value
                    ? 'bg-primary text-primary-foreground'
                    : 'bg-muted/50 text-foreground hover:bg-muted'
                  }`}
              >
                <span className="font-medium">{option.label}</span>
                {autoClearDays === option.value && <Check className="w-5 h-5" />}
              </button>
            ))}
          </div>
        </DialogContent>
