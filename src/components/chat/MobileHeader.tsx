...
export function MobileHeader({ username, pendingCount = 0, unreadCount = 0, onMenuOpen }: MobileHeaderProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 md:hidden bg-card border-b border-border flex items-center justify-between px-4 h-14">
      {/* Hamburger */}
      <button
        onClick={onMenuOpen}
      ...
      <div className="flex items-center gap-2">
        <div className="w-7 h-7 rounded-lg bg-primary/10 flex items-center justify-center overflow-hidden">
          <img src={logoUrl} alt="ShadowCrypt" className="w-5 h-5 object-contain" />
        </div>
        <span className="text-sm font-semibold text-foreground">ShadowCrypt</span>
      </div>
