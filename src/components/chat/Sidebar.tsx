...
function BuyMeACoffee() {
  return (
    <div className="flex justify-center my-2">
      ...
      >
        <img
          src="https://img.buymeacoffee.com/button-api/?text=Buy%20creator%20a%20cup%20of%20coffee&emoji=%E2%98%95&slug=admin.forestritium&button_colour=40DCA5&font_colour=ffffff&font_family=Cookie&outline_colour=000000&coffee_colour=FFDD00"
          alt="Buy creator a cup of coffee"
...
export interface SidebarProps {
  ...
  onContactRemoved?: (contactId: string) => Promise<void>;
  onLogout: () => void;
...
function SidebarContent({
  ...
  const handleBlockContact = async (contact: Contact) => {
    try {
      ...
      await removeContactAndMessagesFromDB(currentUserId, contact.id, convId);
      await deleteContactRequestBetween(currentUserId, contact.id);
      toast.success(`@${contact.username} has been blocked and removed.`);
      onConversationDeselect?.(convId);
      onContactsChange();
      onRequestHandled();
    } catch (err) {
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-xl bg-primary/10 flex items-center justify-center overflow-hidden shrink-0">
                <img src={logoUrl} alt="ShadowCrypt" className="w-6 h-6 object-contain" />
              </div>
              <div>
                <p className="text-sm font-semibold text-sidebar-foreground leading-tight">ShadowCrypt</p>
      ...
      <AddContactDialog
        ...
        onContactAdded={() => { setAddContactOpen(false); onContactsChange(); onRequestHandled(); }}
      />
        ...
        <AlertDialogContent className="max-w-[calc(100%-2rem)] md:max-w-lg">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-pretty">
              ...
              you messages or contact requests. They will also be removed from your contacts.
