...
interface VoiceWaveformProps {
  ...
  audioData?: number[];
  isRecording?: boolean;
  styleOverride?: { type: 'legacy' | 'modern'; color: string };
  className?: string;
}

export function VoiceWaveform({ analyser, audioData, styleOverride }: VoiceWaveformProps) {
  ...
  useEffect(() => {
    ...
    if (!analyser && (!audioData || audioData.length === 0)) return;
    ...
    const draw = () => {
      ...
      const currentType = styleOverride?.type || settings.type;
      if (currentType === 'legacy') {
        if (analyser && typeof analyser.getByteTimeDomainData === 'function') {
           ...
           for (let i = 0; i < bufferLength; i++) {
             dataArray[i] = audioData[Math.floor((i / bufferLength) * audioData.length)] || 128;
           }
        }
      } else {
        if (analyser && typeof analyser.getByteFrequencyData === 'function') {
           ...
           for (let i = 0; i < bufferLength; i++) {
             dataArray[i] = audioData[Math.floor((i / bufferLength) * audioData.length)] || 0;
           }
        }
      }
      ...
      const colorToUse = styleOverride?.color || settings.color;
      if (colorToUse !== 'primary' && colorToUse !== '') {
  ...
  }, [analyser, audioData, styleOverride?.type, styleOverride?.color, settings.type, settings.color]);
