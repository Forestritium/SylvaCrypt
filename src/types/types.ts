...
export interface Profile {
  ...
  bio: string | null;
  bio_private: boolean | string[];      // when true: hide from everyone. false: public. string[]: array of allowed userIds.
  created_at: string;
  ...
  avatar_private: boolean | string[];   // when true: hide from everyone. false: public. string[]: array of allowed userIds.
...
export interface Contact {
  ...
  /** Local-only nickname displayed instead of the contact's username. */
  nickname?: string | null;
  /** Local-only theme ID applied while this contact's chat is open. */
  theme?: string | null;
  avatarUrl?: string | null;
  bio?: string | null;
  /**
   * Number of days after which messages in this conversation are auto-cleared
   * locally.  0 (or absent) means disabled.  Stored in the vault-encrypted
   * contact metadata blob so it is never written to localStorage in cleartext.
   */
  autoClearDays?: number;
  /**
   * True when this conversation's notification sound is muted.
   * Stored in the vault-encrypted contact metadata blob (v2+).
   * Replaces the previous cleartext localStorage key sylvacrypt_muted_convos.
   */
  muted?: boolean;
}
...
export interface LocalMessage {
  ...
  /** Original MIME type of the image (e.g. "image/gif"). Preserved so animated GIFs render correctly after decryption. */
...
export interface RatchetSession {
  ...
  DHs_priv?: string;
  DHs_pub?: string;
  DHs?: string; // Legacy fallback
  DHr: string | null; // their current DH ratchet public key (base64)
  // ML-KEM-768 Ratchet
  KEMs_priv?: string;
  KEMs_pub?: string;
  KEMs?: string; // Legacy fallback
  prevKEMs_priv?: string;
  prevKEMs_pub?: string;
  prevKEMs?: string; // Legacy fallback
  ...
  /** Initial shared secret (base64) — stable for the lifetime of the session. Used to derive the shared notebook key. */
  sharedSecret?: string;
  // Message counters
