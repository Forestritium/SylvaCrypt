...
import {
  ...
  ShieldCheck, LifeBuoy, BookOpen, Coffee, PanelLeftClose
} from 'lucide-react';
import { Toggle } from '@/components/ui/toggle';
import { useTheme } from '@/contexts/ThemeContext';
import { useKeyboardShortcuts } from '@/hooks/useKeyboardShortcuts';
import { exportLocalVault, importLocalVault } from '@/lib/vaultExport';
import { useAuth } from '@/contexts/AuthContext';
import { unblockUser, fetchBlockedUsers, fetchContactKeyHistory } from '@/lib/relay';
import { getMnemonic, getEncryptedIdentityKeyBlob, getStoredSaltBase64, getKdfVersion, getContacts, syncVaultKeyPersistence, restoreVaultFromBackup, setTypingDisabledPref, getTypingDisabledPref } from '@/lib/localStore';
import {
...
type Section = 'main' | 'bio' | 'username' | 'blocklist' | 'delete' | 'recovery' | 'security' | 'key-history' | 'storage' | 'edit-bio' | 'edit-username';
...
export default function SettingsPage() {
  ...
  useKeyboardShortcuts({
    ...
    onNewContact: () => navigate('/home'),
    onFocusMessageInput: () => navigate('/home'),
    onOpenSettings: () => { if (section !== 'main') setSection('main'); }
  });
  ...
  const [avatarPrivate, setAvatarPrivate] = useState<boolean | string[]>(profile?.avatar_private ?? false);
  const [cropSrc, setCropSrc] = useState<string | null>(null);
  const [cropOpen, setCropOpen] = useState(false);

  // Bio
  const [bioInput, setBioInput] = useState(profile?.bio ?? '');
  const [bioPrivate, setBioPrivate] = useState<boolean | string[]>(profile?.bio_private ?? false);
  ...
  // Security / privacy prefs (persisted in vault-encrypted user prefs)
  const [typingDisabled, setTypingDisabled] = useState(
    () => {
      // Read from sessionStorage write-through cache (seeded on vault unlock).
  ...
  // Hide the "Buy creator a cup of coffee" button in the sidebar
  ...
  useEffect(() => {
    ...
    if (section === 'main') getContacts().then(setContacts);
  }, [section]);

  // Hydrate typing-disabled from encrypted store on mount (cache may be cold
  // on first render before vault-unlock seeds sessionStorage).
  useEffect(() => {
    getTypingDisabledPref().then(val => setTypingDisabled(val)).catch(() => {});
  }, []);
  ...
  const handleAvatarPrivacyToggle = async () => {
    ...
    if (avatarPrivate === false) {
      next = true; // Switch to completely private
    } else {
      next = false; // Switch back to public
    }
    setAvatarPrivate(next);
    const { error } = await updateAvatarPrivacy(next as any);
    if (error) { setAvatarPrivate(avatarPrivate); toast.error('Failed to update privacy setting.'); }
    else toast.success('Profile picture privacy updated.');
  };

  // ── Bio ───────────────────────────────────────────────────────────────────
  const handleSaveBio = async () => {
    setSavingBio(true);
    const { error } = await updateBio(bioInput, bioPrivate as any);
    setSavingBio(false);
  ...
  const toggleTyping = () => {
    ...
    // Persist to vault-encrypted user prefs; also updates sessionStorage write-through cache.
    setTypingDisabledPref(next).catch(() => {});
    // Remove legacy cleartext key if it still exists (migration).
    try { localStorage.removeItem('sc_typing_disabled'); } catch { /* ignore */ }
  ...
  const handleExportBackup = async () => {
    ...
    try {
      ...
      const encryptedVaultBlob = await exportLocalVault();
      const bundle = JSON.stringify({
        ...
        // contacts and messages are stored as an AES-256-GCM encrypted blob.
        // The blob is decrypted by importLocalVault() using the vault key,
        // which provides both confidentiality and AEAD integrity protection.
        encrypted_vault_data: encryptedVaultBlob,
  ...
  const handleImportBackup = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Support both v2 (encrypted_vault_data blob) and legacy v1 (plain contacts/messages).
      if (bundle.encrypted_vault_data) {
        await importLocalVault(bundle.encrypted_vault_data);
      } else if (bundle.contacts && bundle.messages) {
        await importLocalVault(JSON.stringify({ contacts: bundle.contacts, messages: bundle.messages }));
      }
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <div className="flex-1 p-4 max-w-lg mx-auto w-full space-y-4">
          ...
          <button onClick={() => {
            if (bioPrivate === false) setBioPrivate(true);
            else if (bioPrivate === true) setBioPrivate([]);
            else setBioPrivate(false);
          }}
            className={`flex items-center gap-2.5 w-full px-4 py-3 rounded-xl border transition-colors ${
              bioPrivate === true
                ? 'bg-amber-500/10 border-amber-500/30 text-amber-600 dark:text-amber-400'
                : Array.isArray(bioPrivate)
                ? 'bg-blue-500/10 border-blue-500/30 text-blue-600 dark:text-blue-400'
                : 'bg-muted border-border text-foreground hover:bg-muted/80'
            }`}>
            {bioPrivate === true ? <Lock className="w-4 h-4 shrink-0" /> : Array.isArray(bioPrivate) ? <User className="w-4 h-4 shrink-0" /> : <Unlock className="w-4 h-4 shrink-0" />}
            <div className="flex-1 text-left">
              <p className="text-sm font-medium leading-tight">
                {bioPrivate === true ? 'Bio is Private' : Array.isArray(bioPrivate) ? 'Bio is Shared Selectively' : 'Bio is Public'}
              </p>
              <p className="text-xs text-muted-foreground mt-0.5">
                {bioPrivate === true ? 'Only you can see your bio' : Array.isArray(bioPrivate) ? 'Only selected contacts can see your bio' : 'Your contacts can see your bio'}
              </p>
            </div>
          </button>
          {Array.isArray(bioPrivate) && (
            <div className="mt-4 border rounded-xl p-3 bg-muted/50">
              <p className="text-sm font-medium mb-2">Allowed Contacts</p>
              {contacts.length === 0 ? (
                <p className="text-xs text-muted-foreground">No contacts yet.</p>
              ) : (
                <div className="flex flex-col gap-2 max-h-40 overflow-y-auto pr-2 custom-scrollbar">
                  {contacts.map(c => (
                    <label key={c.id} className="flex items-center gap-3 text-sm cursor-pointer p-1 hover:bg-muted rounded-md transition-colors">
                      <div className="relative flex items-center justify-center">
                        <input 
                          type="checkbox"
                          checked={Array.isArray(bioPrivate) && bioPrivate.includes(c.id)}
                          onChange={(e) => {
                            if (!Array.isArray(bioPrivate)) return;
                            if (e.target.checked) {
                              setBioPrivate([...bioPrivate, c.id]);
                            } else {
                              setBioPrivate(bioPrivate.filter(id => id !== c.id));
                            }
                          }}
                          className="peer appearance-none w-5 h-5 rounded-md border border-border bg-card hover:border-primary/50 checked:bg-primary checked:border-primary transition-all cursor-pointer"
                        />
                        <div className="pointer-events-none absolute text-primary-foreground opacity-0 peer-checked:opacity-100 transition-opacity">
        ...
        <div className="max-w-lg mx-auto w-full px-4 py-6 space-y-6">
            ...
            <div className="bg-card border border-border rounded-2xl p-5 flex flex-col items-center gap-4">
              ...
              <button onClick={handleAvatarPrivacyToggle}
                className={`flex items-center gap-2.5 w-full px-4 py-3 rounded-xl border transition-colors ${
                  avatarPrivate === true
                    ? 'bg-amber-500/10 border-amber-500/30 text-amber-600 dark:text-amber-400'
                    : Array.isArray(avatarPrivate)
                    ? 'bg-blue-500/10 border-blue-500/30 text-blue-600 dark:text-blue-400'
                    : 'bg-muted border-border text-foreground hover:bg-muted/80'
                }`}>
                {avatarPrivate === true ? <Lock className="w-4 h-4 shrink-0" /> : Array.isArray(avatarPrivate) ? <User className="w-4 h-4 shrink-0" /> : <Unlock className="w-4 h-4 shrink-0" />}
                <div className="flex-1 text-left">
                  <p className="text-sm font-medium leading-tight">
                    {avatarPrivate === true ? 'Picture is Private' : Array.isArray(avatarPrivate) ? 'Picture is Shared Selectively' : 'Picture is Public'}
                  </p>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {avatarPrivate === true ? 'Only you can see your profile picture' : Array.isArray(avatarPrivate) ? 'Only selected contacts can see your profile picture' : 'Your contacts can see your profile picture'}
                  </p>
                </div>
              </button>
              {Array.isArray(avatarPrivate) && (
                <div className="mt-4 border rounded-xl p-3 bg-muted/50">
                  <div className="flex justify-between items-center mb-2">
                    <p className="text-sm font-medium">Allowed Contacts</p>
                    <button 
                      onClick={() => updateAvatarPrivacy(avatarPrivate as any)}
                      ...
                      {contacts.map(c => (
                        <label key={c.id} className="flex items-center gap-3 text-sm cursor-pointer p-1 hover:bg-muted rounded-md transition-colors">
                          <div className="relative flex items-center justify-center">
                            <input 
                              type="checkbox"
                              checked={Array.isArray(avatarPrivate) && avatarPrivate.includes(c.id)}
                              onChange={(e) => {
                                if (!Array.isArray(avatarPrivate)) return;
                                if (e.target.checked) {
                                  setAvatarPrivate([...avatarPrivate, c.id]);
                                } else {
                                  setAvatarPrivate(avatarPrivate.filter(id => id !== c.id));
                                }
                              }}
                              className="peer appearance-none w-5 h-5 rounded-md border border-border bg-card hover:border-primary/50 checked:bg-primary checked:border-primary transition-all cursor-pointer"
                            />
          ...
          <section>
            ...
            <div className="bg-card border border-border rounded-2xl overflow-hidden divide-y divide-border">
              ...
              <div className="w-full flex items-center justify-between px-4 py-3.5 hover:bg-muted/50 transition-colors">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
                    <PanelLeftClose className="w-4 h-4 text-primary" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-foreground">Legacy Sidebar Toggle</p>
                    <p className="text-xs text-muted-foreground">Use standard hamburger menu on mobile instead of side tab</p>
                  </div>
                </div>
                <Toggle
                  pressed={localStorage.getItem('sc_legacy_sidebar') === '1'}
                  onPressedChange={(pressed) => {
                    try {
                      localStorage.setItem('sc_legacy_sidebar', pressed ? '1' : '0');
                      // Force reload to apply layout changes
              ...
              <button
                onClick={() => {
                  const next = !hideBuyCoffee;
                  setHideBuyCoffee(next);
                  try {
                    localStorage.setItem('sc_hide_buy_coffee', next ? '1' : '0');
                    toast.success(next ? '"Buy creator a cup of coffee" hidden.' : '"Buy creator a cup of coffee" shown.');
                  } catch {
                    toast.error('Failed to save preference.');
                  }
                }}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-foreground">Buy creator a cup of coffee</p>
                  <p className="text-xs text-muted-foreground">{hideBuyCoffee ? 'Hidden from sidebar' : 'Shown in sidebar'}</p>
                </div>
                {hideBuyCoffee ? <ToggleRight className="w-5 h-5 text-primary shrink-0" /> : <ToggleLeft className="w-5 h-5 text-muted-foreground shrink-0" />}
              </button>
            </div>
          </section>
            ...
            <div className="bg-card border border-border rounded-2xl overflow-hidden divide-y divide-border">
              ...
              <button onClick={() => setSection('recovery')} className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-muted/50 transition-colors text-left">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">View or regenerate your 24-word backup phrase</p>
                </div>
                <ChevronRight className="w-4 h-4 text-muted-foreground shrink-0" />
              </button>
