...
export default function ChatPage() {
  ...
  const swipeHandlers = useSwipeable({
    onSwipedRight: () => {
      if (!isLegacySidebar && !sidebarOpen) {
        setSidebarOpen(true);
      }
    },
    trackMouse: true, // Allow mouse swiping for testing
    delta: 30, // Minimum swipe distance in pixels
  });
  ...
  return (
    <div className="flex flex-col md:flex-row h-[100dvh] w-full bg-background overflow-hidden">
      ...
      {/* Main chat area */}
      <div {...(!isLegacySidebar && !sidebarOpen ? swipeHandlers : {})} className={`flex-1 min-w-0 flex flex-col overflow-hidden relative`}>
        
        {/* Mobile sidebar expand button - replacing hamburger menu (if legacy sidebar is disabled) */}
        {!isLegacySidebar && (
