...
import { getLastMessageTimes } from '@/lib/dbStore';
...
export default function ChatPage() {
  ...
  // Swipe-right to open sidebar — only active when no conversation is open.
  // When a conversation IS open, swipe-right is used for swipe-to-reply inside
  // ChatArea, so we must never intercept it here.
  const swipeHandlers = useSwipeable({
    onSwipedRight: () => {
      if (!isLegacySidebar && !sidebarOpen && !selectedConversation) {
        setSidebarOpen(true);
      }
    },
    trackMouse: false, // Do not intercept mouse; touch only
    delta: 50, // Require a more deliberate swipe to avoid accidental triggers
  ...
  useEffect(() => {
    // Check if we navigated to /home to clear the active conversation
  ...
  const loadLocalData = useCallback(async () => {
    ...
    try {
      ...
      const [c, lmt, lmp] = await Promise.all([
        getContactsFromDB(user.id),
        getLastMessageTimes(user.id),
        import('@/lib/dbStore').then(m => m.getLastMessagePreviews(user.id))
      ]);
      setContacts(c);
      setLastMessageTimes(lmt);
      setLastMessagePreviews(lmp);
      // Refresh public keys from live profiles, detect any key changes, and
      // surface them as blocking alerts in ChatArea.
      if (c.length > 0) {
        const [updatedKeys, alerts] = await refreshContactPublicKeys(user.id, c.map((ct: any) => ({
          id: ct.id,
  ...
  const handleIncomingRelay = useCallback(async (relayMsg: RelayMessage) => {
    ...
    if (localMsg) {
      ...
      setLastMessageTimes(prev => ({
        ...prev,
        [localMsg.conversationId]: localMsg.timestamp,
      }));
      setLastMessagePreviews(prev => ({
        ...prev,
        [localMsg.conversationId]: localMsg.imageUrl ? '📷 Photo' : (localMsg.content.includes('data:audio/') ? '🎤 Voice message' : localMsg.content),
      }));
  ...
  return (
    <div className="flex flex-col md:flex-row h-[100dvh] w-full bg-background overflow-hidden">
      ...
      {!selectedConversation && (
        <div className="md:hidden">
          <MobileHeader
            ...
            avatarUrl={(session as any).avatar_url || (session as any).avatarUrl}
      ...
      {/* Mobile sidebar (full screen content below header) */}
      {!selectedConversation && (
        <div className="md:hidden flex-1 overflow-hidden hidden">
          <Sidebar
            contacts={contacts}
            ...
            profile={profile}
            isOpen={false} // don't open the Drawer version
      ...
      {!desktopSidebarCollapsed && (
        <div className="hidden md:block h-full">
          <Sidebar
            ...
            onConversationDeselect={(convId: string) => {
      ...
      {/* Main chat area */}
      {/* swipeHandlers intentionally NOT spread when a conversation is open — swipe-right belongs to swipe-to-reply in ChatArea */}
      <div {...(!isLegacySidebar && !sidebarOpen && !selectedConversation ? swipeHandlers : {})} className="flex-1 min-w-0 flex flex-col overflow-hidden relative">
        
        {/* Mobile sidebar expand button - replacing hamburger menu (if legacy sidebar is disabled). Hidden when a chat is active so it does not conflict with swipe-to-reply. */}
        {!isLegacySidebar && !selectedConversation && (
          <div className="absolute top-1/2 left-0 -translate-y-1/2 z-10 md:hidden">
            <button
              onClick={() => setSidebarOpen(true)}
              className="h-16 w-4 bg-card border border-border border-l-0 rounded-r-xl flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted transition-colors shadow-sm"
            >
              <ChevronRight className="w-3 h-3" />
              {/* Show badge indicator on the tab if there are unread messages or pending requests */}
              {(pendingRequests.length > 0 || Object.values(unreadCounts).reduce((s, n) => s + n, 0) > 0) && (
