              ...
              {devices.map(device => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Row 2: fingerprint */}
                    <div className="bg-muted/60 rounded-xl px-3 py-2.5 relative group">
                      <div className="flex items-center justify-between gap-2 mb-1">
                        <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                          Key Fingerprint
                        </p>
                        <button
                          onClick={() => handleCopyFingerprint(device.id, fp)}
                          className={`text-muted-foreground transition-colors ${copiedFingerprint === device.id ? 'text-primary' : 'hover:text-primary'}`}
                          aria-label="Copy fingerprint"
                        >
                          {copiedFingerprint === device.id ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
                        </button>
                      </div>
                      <p className="font-mono text-[11px] text-primary/80 break-all leading-relaxed tracking-wide select-all">
                        {fp}
                      </p>
                    </div>
